...
function cassiopeia_admin_theme_white_preprocess_html(&$variables) {
    global $user;
    if($user->uid!=1){
        $variables['classes_array'][] = 'hold-transition autic-theme sidebar-mini admin-menu sidebar-collapse';
    }else{
        $variables['classes_array'][] = 'hold-transition autic-theme sidebar-mini admin-menu ';
    }
...
function cassiopeia_admin_theme_white_preprocess_page(&$variables) {
  $_arg = arg();
...
function cassiopeia_admin_theme_white_preprocess_admin_block(&$vars) {
  // Add icon and classes to admin block titles.
  if (isset($vars['block']['href'])) {
    $vars['block']['localized_options']['attributes']['class'] = _cassiopeia_admin_theme_white_icon_classes($vars['block']['href']);
  }
...
function cassiopeia_admin_theme_white_admin_block($variables) {
...
function cassiopeia_admin_theme_white_admin_block_content($vars) {
  ...
  if (!empty($content)) {
    ...
    foreach ($content as $k => $item) {
      ...
      if (!empty($content[$k]['localized_options']['attributes']['class'])) {
        $content[$k]['localized_options']['attributes']['class'] += _cassiopeia_admin_theme_white_icon_classes($item['href']);
      }
      else {
        $content[$k]['localized_options']['attributes']['class'] = _cassiopeia_admin_theme_white_icon_classes($item['href']);
      }
...
function _cassiopeia_admin_theme_white_icon_classes($path) {
  $classes = array();
  $args = explode('/', $path);
