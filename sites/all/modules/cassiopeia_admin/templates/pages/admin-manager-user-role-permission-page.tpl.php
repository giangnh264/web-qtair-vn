        ...
        <div class="application-user-permissions-page-header navbar navbar-default">
            <ul class="nav navbar-nav">
                <?php foreach($permissions_list as $key => $value): ?>
                    <li><a class="<?php if ($type==$key){print('active');} ?>" href="<?php print('/admin/manager/'.('user/role/'.$role->rid).'/permissions?type='.$key); ?>"><?php echo strtoupper($key); ?>. <?php echo $value['title']; ?></a></li>
                <?php endforeach; ?>
            </ul>
        </div>
        ...
        <div class="application-user-permissions-page-body">
            <div class="box box-solid">
                <div class="box-body" style="padding: 10px;">
                    <?php
                    $cassiopeia_user_permissions_form = drupal_get_form('cassiopeia_user_role_permissions_form', array('role'=>$role, 'type'=>$type));
