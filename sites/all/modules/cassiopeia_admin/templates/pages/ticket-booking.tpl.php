<?php
//db_delete("tbl_booking")->condition("booking_code","VE1600097396")->execute();
global $user;
//f
try{
    db_update("tbl_booking")->fields(array(
        ...
        ->condition("ExpiryDt",0,"<>")
        ->condition("ExpiryDt",REQUEST_TIME,"<=")
        ->condition("status","TICKETED","<>")
        ->condition("status","OK","=")
        ->execute();
}catch (Exception $e){
    print_r($e);
...
?>
    <div class="page-admin-manager-booking">
        ...
        try{
            ...
            // _print_r(array(strtotime(date("01-m-Y 00:00",REQUEST_TIME)),strtotime(date("t-m-Y 23:59",REQUEST_TIME))),"BETWEEN");
        ...
        <div id="modal_receipts_form" class="modal fade" role="dialog">
            <div class="modal-dialog modal-lg">
                <!-- Modal content-->
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <div class="block-container">
                            <?php
                            $cassiopeia_receipts_form = drupal_get_form("cassiopeia_receipts_form");
                            if(!empty($cassiopeia_receipts_form)){
                                $cassiopeia_receipts_form = drupal_render($cassiopeia_receipts_form);
                                print($cassiopeia_receipts_form);
                            }
                            ?>
                        </div>
                    </div>
                    <!--                <div class="modal-footer">-->
                    <!--                    <button class="btn btn-primary btn-create-receipts">Lập phiếu thu</button>-->
                    <!--                </div>-->
                </div>
