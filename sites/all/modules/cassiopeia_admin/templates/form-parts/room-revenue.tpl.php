...
//_print_r($cache);
try{
    $daily_sub_query = db_select("tbl_room_booking_report","tbl_room_booking_report");
    $daily_sub_query->fields("tbl_room_booking_report",array("created"));
    $daily_sub_query->addExpression("1","type");
    $daily_sub_query->addField("tbl_room_booking_report","created","date");
    $daily_sub_query->addField("tbl_room_booking_report","partner_price","partner_price");
    $daily_sub_query->addField("tbl_room_booking_report","price","price");
    $daily_sub_query->addField("tbl_room_booking_report","quantity_night","quantity_night");
    ...
    if(!empty($date_filter && $date_filter!="all")){
        ...
        $daily_sub_query ->condition("created",array($first_day_of_moth,$last_day_of_moth),"BETWEEN");
    }

    $daily_sub_query->addExpression("DATE_FORMAT(FROM_UNIXTIME(created), '%d-%m-%Y') ","cr");

    $daily_query = db_select($daily_sub_query,"tbl_daily_query");
    $daily_query->fields("tbl_daily_query");
    $daily_query->groupBy("cr");
    $daily_query->addExpression("SUM(tbl_daily_query.quantity_night)","total_quantity");
