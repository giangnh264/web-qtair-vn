        ...
        <table class="table table-hover">
            ...
            <?php for($i=1;$i<=date("t", strtotime("01-".$date_filter));$i++): ?>
                ...
                try{
                    ...
                    $query->leftJoin("field_data_field_account_sale","tbl_sale","tbl_sale.entity_id=tbl_issue_report.agent");
                    $query->addField("tbl_sale","field_account_sale_target_id","sale");
                    $query->condition("tbl_issue_report.date",array(strtotime($i.date("-m-Y 00:00",strtotime("01-".$date_filter))),strtotime($i.date("-m-Y 23:59",strtotime("01-".$date_filter)))),"BETWEEN");
