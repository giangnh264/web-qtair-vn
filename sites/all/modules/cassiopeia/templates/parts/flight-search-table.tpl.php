...
$point_1 = cassiopeia_get_airport($info->StartPoint);
$point_2 = cassiopeia_get_airport($info->EndPoint);
$__Departure    = date("Y-m-d",strtotime(str_replace("/","-",$info->DepartureDate)));
$__ReturnDate    = date("Y-m-d",strtotime(str_replace("/","-",$info->ReturnDate)));
$departures = array();
$returns = array();
$airports = array();
$airlines = array();
...
?>
<input type="hidden" id="flight_type" class="123" value="<?php print(!empty($returns)?2:1); ?>">
    <?php if(!empty($FlightGroups)): ?>
        <?php foreach($FlightGroups as $GroupSession => $FlightGroup): ?>
            <?php
				...
				$ReturnFlight = array_values((array)$FlightGroup->ReturnFlights)[0];
                $region_fee = 0;
                if (in_array($DepartureFlight->AirlineCode, $Domestic_airlines)) {
                    $domesticAirline = true;
                }
                if(!empty($ReturnFlight)){
                    ...
                    $service_fee_departure = cassiopeia_user_service_fee_load_by_class($DepartureFlight->AirlineCode,$DepartureFlight->ListSegment[0]->Class);
                    $region_fee += $agent_fee['international_fee'];
                // $region_fee += $service_fee['international_fee'];
                    if($domesticAirline){
                        $region_fee += $service_fee_departure['international_domestic_fee'];
                    }else{
                        $region_fee += $service_fee_departure['international_fee'];
                    }
                    $agent_fee = cassiopeia_user_agent_fee_load_by_class($ReturnFlight->AirlineCode,$ReturnFlight->ListSegment[0]->Class);
                    $service_fee_return = cassiopeia_user_service_fee_load_by_class($ReturnFlight->AirlineCode,$ReturnFlight->ListSegment[0]->Class);
                    $region_fee += $agent_fee['international_fee'];
                    // $region_fee += $service_fee['international_fee'];
                    if($domesticAirline){
                        $region_fee += $service_fee_return['international_domestic_fee'];
                    }else{
                        $region_fee += $service_fee_return['international_fee'];
                    }
...
//                    print_r($service_fee);
                    ...
                    if($domesticAirline){
                        $region_fee += $service_fee['international_domestic_fee'];
                    }else{
                        $region_fee += $service_fee['international_fee'];
                    }
                }
                    ...
                    <div class="flights">
                        <div class="item">
                            <div class="item-title">
                                <div>
                                    ...
                                    <?php if($language->language=="vi"): ?>
                                        <?php if(!empty($point_1)) print(!empty($point_1->city_vi)?$point_1->city_vi:$point_1->city); ?> <i class="fa fa-long-arrow-right"></i> <?php if(!empty($point_2)) print(!empty($point_2->city_vi)?$point_2->city_vi:$point_2->city); ?>
                                    <?php else: ?>
                                        <?php if(!empty($point_1)) print($point_1->city); ?> <i class="fa fa-long-arrow-right"></i> <?php if(!empty($point_2)) print($point_2->city); ?>
                                    <?php endif; ?>
                                </div>
                                <div>
                                    <?php print(_cassiopeia_get_day_off_week(strtotime($__Departure))); ?>, <?php print(date("d/m/Y",strtotime($__Departure))); ?>
                                </div>
                            </div>
                            <?php $stt=1;?>
                            <?php foreach($FlightGroup->DepartureFlights as $FlightSession => $Flight): ?>
                                ...
                                ?>
                                <div class="item-body <?php print($stt==1?"chosen":""); ?>" data-Stops="<?php print($Flight->Stops); ?>" data-item='<?php print(json_encode($Flight)); ?>'>
                                    <div class="journey">
										<div class="logo">
										...
										<div class="startpoint">
											...
											<div>
												<?php if(!empty($startpoint)) echo($startpoint->city."(".$startpoint->code.")"); ?>
											</div>
										</div>
										<div class="between">
											<div class="stop-num">
										...
										<div class="endpoint">
											...
											<div>
												<?php if(!empty($endpoint)) print($endpoint->city."(".$endpoint->code.")");?>
											</div>
										</div>
										<div class="check">
											<input <?php print($stt==1?"checked":""); ?> type="radio" name="radio-departure-flight-<?php print($GroupSession); ?>" class="radio-departure-flight " value="" data-GroupSession="<?php echo $GroupSession; ?>" data-FlightSession="<?php echo $FlightSession; ?>">
										</div>
									</div>
                                    <div class="detail">
                                            ...
                                            <div class="block-container">
                                                <div class="col-md-3">
                                                    <?php if(!empty($Segment_airline)): ?>
                                                        ...
                                                        <div class="airline">
                                                            <?php if(!empty($Segment_airline)) print($Segment_airline->name); ?>
                                                        </div>
                                                    <?php endif; ?>
                                                </div>
                                                <div class="col-md-3">
                            ...
                            <div class="item">
                                <div class="item-title">
                                    <div>
                                        <span class="fa fa-plane"></span>
                                        <?php if($language->language=="vi"): ?>
                                            <?php if(!empty($point_2)) print(!empty($point_2->city_vi)?$point_2->city_vi:$point_2->city); ?> <i class="fa fa-long-arrow-right"></i> <?php if(!empty($point_1)) print(!empty($point_1->city_vi)?$point_1->city_vi:$point_1->city); ?>
                                        <?php else: ?>
                                            <?php if(!empty($point_2)) print($point_2->city); ?> <i class="fa fa-long-arrow-right"></i> <?php if(!empty($point_1)) print($point_1->city); ?>
                                        <?php endif; ?>
                                    </div>
                                    <div>
                                        <?php print(_cassiopeia_get_day_off_week(strtotime($__ReturnDate))); ?>, <?php print(date("d/m/Y",strtotime($__ReturnDate))); ?>
                                    </div>
                                </div>
                                <?php $stt=1;?>
                        ...
                        <div class="block-body">
                            <table class="table table-hovered">
                                ...
                                <tr>
                                    ...
                                    <!--                            <td>--><?php //print(number_format(($item->TaxAdt+$item->FeeAdt+$region_fee),0,",",".")); ?><!--</td>-->
                                ...
                                <tr>
                                    ...
                                    <!--                            <td>--><?php //print(number_format(($item->TaxChd+$item->FeeChd+$region_fee)*$item->Chd,0,",",".")); ?><!--</td>-->
...
<!--                                --><?php //endif; ?>
                                <tr>
                                    ...
                                    <!--                            <td>--><?php //print(number_format(($item->TaxInf+$item->FeeInf+$region_fee)*$item->Inf,0,",",".")); ?><!--</td>-->
